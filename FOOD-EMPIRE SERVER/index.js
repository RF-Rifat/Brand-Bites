...
async function run() {
  try {
    ...
    const brandCollection = client.db("brandDB").collection("food");
    const userCartCollection = client.db("brandDB").collection("userCartData");
    ...
    app.put("/userCartData/:id", async (req, res) => {
      const id = req.params.id;
      ...
      const result = await brandCollection.updateOne(filter, food, options);
...
app.get("/", (req, res) => {
  res.send("<h1>Brand Shop Server</h1>");
});
